...
export default function ManageStudents() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post(`${apiUrl}/users`, { ...formData, role: 'student' }, { withCredentials: true });
      ...
      <div className="bg-white rounded-2xl p-4 md:p-6 border border-slate-200 shadow-sm">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input required type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full rounded-lg border-slate-300 py-2 px-3 border focus:ring-2 focus:ring-blue-600 focus:outline-none" placeholder="Jane Doe" />
          </div>

          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">User ID</label>
            <input required type="text" value={formData.userId} onChange={(e) => setFormData({...formData, userId: e.target.value})} className="w-full rounded-lg border-slate-300 py-2 px-3 border focus:ring-2 focus:ring-blue-600 focus:outline-none" placeholder="student123" />
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">Must exactly match the teacher's batch name to see announcements.</p>
          </div>
